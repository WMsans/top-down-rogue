...
func test_world_to_sector_origin() -> void:
	...
	grid.init_args(12345, _make_biome())
...
func test_resolve_sector_deterministic() -> void:
	...
	var g1 := SectorGrid.new()
	g1.init_args(99999, b)
	var g2 := SectorGrid.new()
	g2.init_args(99999, b)
...
func test_resolve_sector_seed_changes() -> void:
	...
	for x in range(-5, 5):
		for y in range(-5, 5):
			var c := Vector2i(x, y)
			if g1.chebyshev_distance(c, Vector2i.ZERO) >= SectorGrid.get_boss_ring_distance():
				continue
